/*
 * mistakes_codes.h
 *
 *  Created on: Mar 27, 2020
 *      Author: Андрей
 */
#ifndef IMPLEMENTATION_MISTAKES_CODES_H_
	#define IMPLEMENTATION_MISTAKES_CODES_H_

//*** MCU related mistakes ***//

#ifndef MCU_MISTAKES_OFFSET
	#define MCU_MISTAKES_OFFSET				(0U)
#endif

#define HSE_FAILED_TO_START						(1U) + MCU_MISTAKES_OFFSET
#define HSE_FAILED_WHILE_RUNNING 				(2U) + MCU_MISTAKES_OFFSET
#define	PLL_FAILED								(3U) + MCU_MISTAKES_OFFSET
#define SYSCLK_BASIC_SETUP_FAILED				(4U) + MCU_MISTAKES_OFFSET
#define WRONG_SPI3_FREQUENCY_INPUT				(5U) + MCU_MISTAKES_OFFSET
#define SPI3_TRANSMISSION_FAIL					(6U) + MCU_MISTAKES_OFFSET

#define WRONG_UART1_FREQUENCY_INPUT				(9U) + MCU_MISTAKES_OFFSET
#define UART1_TRANSMISSION_FAIL					(10U) + MCU_MISTAKES_OFFSET
//
//*** Motor related mistakes ***//
#define MOTORS_MISTAKES_OFFSET				(300U)
#define MOTOR_PWM_TASK_LOWER_THAN_MINIMUM		(1U) + MOTORS_MISTAKES_OFFSET
#define MOTOR_PWM_TASK_HIGHER_THAN_MAXIMUM		(2U) + MOTORS_MISTAKES_OFFSET
#define MOTOR_IS_NOT_CONNECTED					(5U) + MOTORS_MISTAKES_OFFSET
#define MOTOR_DIRECTION_MISMATCH				(7U) + MOTORS_MISTAKES_OFFSET




#endif /* IMPLEMENTATION_MISTAKES_CODES_H_ */
